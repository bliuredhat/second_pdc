...
module MessageBus
  class Handler
    ...
    def initialize(url=MessageBus::BROKER_URL)
      ...
      unless @cert.nil?
        MBUSLOG.info("Using cert #{@cert} & key #{@key} for message bus authentication")
      end
    ...
    def topic_send(topic, content, properties={})
      ...
      begin
        MBUSLOG.debug "Sending message to: #{msg.address}"
        MBUSLOG.debug "Message header: #{msg.properties}"
        MBUSLOG.debug "Message body: #{msg.body}"
        ...
        MBUSLOG.debug "Send message to #{msg.address} successfully."
      ensure
    ...
    def broker
      @broker_urls.each do |broker_url|
        begin
          ...
          MBUSLOG.warn "#{broker_url} failed connection test: #{e.inspect}"
        end
      end
    ...
    def build_message(topic)
      ...
      msg.subject = topic
      msg.durable = true
      msg.address = Settings.mbus_eng_topic + "#{topic}"
    ...
    def subscribe(address, props, &block)
      unless @subs.include? address
        MBUSLOG.info "Will subscribe to #{address}"
      end
      @subs[address] ||= []
    ...
    def listen
      ...
      @broker_urls.cycle.each do |url|
        ...
        if error.nil?
          ...
          MBUSLOG.info "Exiting without error."
          return
        end
        ...
        if attempt > max
          ...
          MBUSLOG.error "Giving up due to repeated failures from every configured broker."
          raise error
        end
    ...
    def listen_with_broker(url)
      MBUSLOG.info "Listening for messages from broker #{url}"

      begin
        test_connection!(url)
      rescue StandardError => e
        MBUSLOG.warn "#{url} failed connection test: #{e.inspect}"
        return e
      end
    ...
    def test_connection!(url)
      ...
      socket = nil
      begin
        MBUSLOG.debug "Testing connection to #{u.host}:#{u.port}..."
        socket = TCPSocket.new(u.host, u.port)
        MBUSLOG.debug "Connected OK."
      ensure
        unless socket.nil?
          begin
            MBUSLOG.debug "Closing socket after connection test..."
            socket.close
            MBUSLOG.debug "Closed socket OK."
          rescue StandardError => e
    ...
    def nonblock_stop(messenger)
      ...
      begin
        messenger.stop unless messenger.stopped?
      rescue StandardError => e
    ...
    def log_error(prefix, e)
      MBUSLOG.error "#{prefix}: #{e.inspect}\n#{e.backtrace.map{|line| "  #{line}"}.join("\n")}"
    end
    ...
    def new_messenger(url)
      ...
      messenger.timeout = Settings.mbus_work_interval * 1000
    ...
    def dispatch_all(broker_url, messenger)
      ...
      while messenger.incoming > 0
        ...
        MBUSLOG.debug "Received: #{message}"
    ...
    def warn_unmatched(message)
      MBUSLOG.warn [
        ...
        "  ID:      #{message.id}",
        "  Address: #{message.address.inspect.truncate(120)}",
        "  Props:   #{message.properties.inspect.truncate(120)}",
      ].join("\n")
    end
