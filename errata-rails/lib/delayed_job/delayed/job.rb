...
module Delayed
  ...
  class Job < ActiveRecord::Base
    ...
    def reschedule(message, backtrace = [], time = nil)
      # Needed due to
      # https://bugzilla.redhat.com/show_bug.cgi?id=704271
    ...
    def finish
      ...
      if obj.respond_to?(:rerun?) && obj.rerun?
        next_run = obj.next_run_time if obj.respond_to?(:next_run_time)
        next_run ||= 10.minutes.from_now
        self.update_attributes!(:locked_at => nil, :locked_by => nil, :failed_at => nil, :last_error => nil, :updated_at => Time.now, :run_at => next_run)
      else
        self.destroy
      end
    end
